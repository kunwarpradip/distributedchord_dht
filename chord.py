...
class Local(object):
	...
	def shutdown(self):
		self.shutdown_ = True
		try:
			if hasattr(self, 'socket_') and self.socket_:
	...
	# logging function
	def log(self, info):
		with open("/tmp/chord.log", "a+", encoding="utf-8") as f:
	...
	def join(self, remote_address = None):
		...
		self.finger_ = [None for _ in range(LOGSIZE)]
	...
	def fix_fingers(self):
		...
		try:
			...
			# Silently ignore errors (node might be shutting down)
			pass
		# Keep calling us
		return True
	...
	def run(self):
		...
		while 1:
			...
			try:
				...
				# Silently ignore simulated network errors (packet drop, partition)
	...
	def unregister_command(self, cmd):
		self.command_ = [t for t in self.command_ if t[0] != cmd]
